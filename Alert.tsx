...
const Alert: React.FC<AlertProps> = ({
  ...
  const getIcon = () => {
    switch (type) {
      case 'warning':
        return <AlertTriangle className={`h-5 w-5 ${styles.icon} mt-0.5 mr-2`} />;
      case 'error':
        return <AlertCircle className={`h-5 w-5 ${styles.icon} mt-0.5 mr-2`} />;
      case 'success':
        return <CheckCircle className={`h-5 w-5 ${styles.icon} mt-0.5 mr-2`} />;
      default: // info
        return <Info className={`h-5 w-5 ${styles.icon} mt-0.5 mr-2`} />;
    }
  };
      ...
      <div className="flex items-start">
        ...
        <div>
          ...
          {actionButton && (
            ...
            >
              {actionButton.active ? `✓ ${actionButton.label}` : actionButton.label}
            </button>
          )}
        </div>
      </div>
